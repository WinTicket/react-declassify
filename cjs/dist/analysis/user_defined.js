...
exports.analyzeUserDefined = analyzeUserDefined;
exports.postAnalyzeCallbackDependencies = postAnalyzeCallbackDependencies;
...
function analyzeUserDefined(instanceFields, softErrors) {
  ...
  for (const [name, field] of instanceFields) {
    ...
    if (initSite) {
      ...
      if ((0, _utils.isClassMethodLike)(initSite.path)) {
        fnInit = {
          type: "method",
          path: initSite.path
        };
        ...
        if (initPath.isCallExpression()) {
          ...
          const initRef = (0, _lib.analyzeLibRef)(initFn);
          if (initRef && (0, _lib.isReactRef)(initRef) && initRef.name === "createRef") {
            if (initArgs.length > 0) {
              throw new _error.AnalysisError("Extra arguments to createRef");
            }
            isRefInit = true;
            const typeParameters = (0, _utils.nonNullPath)(initPath.get("typeParameters"));
            if (typeParameters) {
    ...
    if (typeSite) {
      ...
      if (typing.type === "type_value") {
        if (typing.valueTypePath.isTSTypeReference()) {
          ...
          const typeParameters = (0, _utils.nonNullPath)(typing.valueTypePath.get("typeParameters"));
    ...
    for (const site of field.sites) {
      if (site.type === "expr" && site.owner != null) {
        ...
        if (ownerField?.type === "user_defined_function") {
          (0, _utils.getOr)(preDependencies, site.owner, () => []).push(name);
        }
      }
      if (site.type === "expr") {
  ...
  function addReorderedField(name) {
    ...
    if (reorderVisited.has(name)) {
      throw new _error.AnalysisError("Recursive dependency in memoized methods");
    }
...
function postAnalyzeCallbackDependencies(userDefined, props, states, instanceFields) {
