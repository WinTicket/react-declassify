...
exports.preanalyzeClass = preanalyzeClass;
...
function preanalyzeClass(path) {
  ...
  const superClass = (0, _utils.nonNullPath)(path.get("superClass"));
  if (!superClass) {
    // Not a subclass
    return;
  }
  const superClassRef = (0, _lib.analyzeLibRef)(superClass);
  if (
  // Subclass of an unknown class
  !superClassRef ||
  // Not a react thing, presumably
  !(0, _lib.isReactRef)(superClassRef) ||
  ...
  const typeParameters_ = (0, _utils.nonNullPath)(path.get("typeParameters"));
  ...
  if (members) {
    for (const member of members) {
      if (member.isTSPropertySignature() || member.isTSMethodSignature()) {
        const name = (0, _utils.memberName)(member.node);
        if (name != null) {
