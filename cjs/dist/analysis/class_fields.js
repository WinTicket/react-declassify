...
exports.addClassFieldError = addClassFieldError;
exports.analyzeClassFields = analyzeClassFields;
 ...
 */
function analyzeClassFields(path, softErrors) {
  const instanceFields = new Map();
  const getInstanceField = name => (0, _utils.getOr)(instanceFields, name, () => ({
    sites: []
  }));
  const staticFields = new Map();
  const getStaticField = name => (0, _utils.getOr)(staticFields, name, () => ({
    sites: []
  }));
  ...
  for (const itemPath of path.get("body").get("body")) {
    if ((0, _utils.isNamedClassElement)(itemPath)) {
      // The element is a class method or a class field (in a general sense)
      const isStatic = itemPath.node.static;
      const name = (0, _utils.memberName)(itemPath.node);
      if (name == null) {
        if (isStatic) {
          throw new _error.AnalysisError(`Unnamed class element`);
        } else {
      ...
      if ((0, _utils.isClassPropertyLike)(itemPath)) {
        // Class field.
        // - May have an initializer: `foo = 42;` or not: `foo;`
        // - May have a type annotation: `foo: number;` or not: `foo;`
        const valuePath = (0, _utils.nonNullPath)(itemPath.get("value"));
      ...
      } else if ((0, _utils.isClassMethodOrDecl)(itemPath)) {
        ...
        if (kind === "method") {
          field.sites.push({
            ...
            init: (0, _utils.isClassMethodLike)(itemPath) ? {
              type: "init_method",
              methodPath: itemPath
            } : undefined,
            hasWrite: undefined,
          ...
          // Analysis for step 2
          if ((0, _utils.isClassMethodLike)(itemPath)) {
          ...
          throw new _error.AnalysisError(`Not implemented yet: getter / setter`);
          ...
          throw new _error.AnalysisError(`Not implemented yet: ${kind}`);
        }
      } else if ((0, _utils.isClassAccessorProperty)(itemPath)) {
        throw new _error.AnalysisError(`Not implemented yet: class accessor property`);
      }
    } else if ((0, _utils.isStaticBlock)(itemPath)) {
      throw new _error.AnalysisError(`Not implemented yet: static block`);
    } else if (itemPath.isTSIndexSignature()) {
      // Ignore
    } else {
      throw new _error.AnalysisError(`Unknown class element`);
    }
  }
  ...
  if (constructor) {
    ...
    if (constructor.node.params.length > 1) {
      throw new _error.AnalysisError(`Constructor has too many parameters`);
    } else if (constructor.node.params.length < 1) {
      throw new _error.AnalysisError(`Constructor has too few parameters`);
    }
    const param = constructor.node.params[0];
    if (param.type !== "Identifier") {
      throw new _error.AnalysisError(`Invalid constructor parameters`);
    }
    ...
    if (superCallIndex === -1) {
      throw new _error.AnalysisError(`No super call`);
    } else if (superCallIndex > 0) {
      throw new _error.AnalysisError(`No immediate super call`);
    }
    const superCall = stmts[superCallIndex];
    const superCallArgs = superCall.node.expression.arguments;
    if (superCallArgs.length > 1) {
      throw new _error.AnalysisError(`Too many arguments for super()`);
    } else if (superCallArgs.length < 1) {
      throw new _error.AnalysisError(`Too few arguments for super()`);
    }
    const superCallArg = superCallArgs[0];
    if (superCallArg.type !== "Identifier" || superCallArg.name !== param.name) {
      throw new _error.AnalysisError(`Invalid argument for super()`);
    }
    ...
    for (const stmt of initStmts) {
      if (!(stmt.node.type === "ExpressionStatement" && stmt.node.expression.type === "AssignmentExpression" && stmt.node.expression.operator === "=" && stmt.node.expression.left.type === "MemberExpression" && stmt.node.expression.left.object.type === "ThisExpression")) {
        throw new _error.AnalysisError(`Non-analyzable initialization in constructor`);
      }
      const exprPath = stmt.get("expression");
      const name = (0, _utils.memberRefName)(stmt.node.expression.left);
      if (name == null) {
        throw new _error.AnalysisError(`Non-analyzable initialization in constructor`);
      }
  ...
  function traverseItem(owner, path) {
    traverseThis(path, thisPath => {
      ...
      if (!thisMemberPath.isMemberExpression({
        ...
        if (thisMemberPath.isCallExpression() && thisMemberPath.node.arguments[0] === thisPath.node && thisMemberPath.node.callee.type === "MemberExpression" && (0, _utils.memberRefName)(thisMemberPath.node.callee) === "bind" && thisMemberPath.node.callee.object.type === "MemberExpression" && thisMemberPath.node.callee.object.object.type === "ThisExpression") {
          bindThisSites.push({
      ...
      const name = (0, _utils.memberRefName)(thisMemberPath.node);
      if (name == null) {
        softErrors.addThisError(thisPath);
        return;
      }
  ...
  for (const [name, field] of instanceFields) {
    field.sites = field.sites.filter(site => {
      if (site.type === "decl" && site.init?.type === "init_value") {
        ...
        if (bindThisSite && !bindThisSite.bindsMore && (0, _utils.memberRefName)(bindThisSite.bindeePath.node) === name) {
  ...
  for (const [name, field] of instanceFields) {
    ...
    if (numTypes > 1) {
      throw new _error.AnalysisError(`${name} is declared more than once`);
    }
  }
  for (const [name, field] of staticFields) {
    ...
    const numInits = field.sites.reduce((n, site) => n + Number(!!site.init), 0);
    if (numInits > 1) {
      throw new _error.AnalysisError(`static ${name} is initialized more than once`);
    }
    const numTypes = field.sites.reduce((n, site) => n + Number(!!site.typing), 0);
    if (numTypes > 1) {
      throw new _error.AnalysisError(`static ${name} is declared more than once`);
    }
  }
  return {
    instanceFields,
...
function addClassFieldError(site, softErrors) {
  if (site.type === "decl") {
    if ((0, _utils.isNamedClassElement)(site.path)) {
