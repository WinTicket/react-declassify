import { assignReturnType, assignTypeAnnotation, assignTypeArguments, assignTypeParameters, importName, isClassMethodLike, isClassPropertyLike, isTS, memberFromDecl, nonNullPath } from "./utils.js";
import { AnalysisError, analyzeClass, preanalyzeClass, needsProps, needAlias, SoftErrorRepository } from "./analysis.js";
...
function transformClass(analysis, softErrors, options, babel) {
  ...
  for (const field of analysis.state.states.values()) {
    ...
    preamble.push(t.variableDeclaration("const", [t.variableDeclarator(t.arrayPattern([t.identifier(field.localName), t.identifier(field.localSetterName)]), ts && field.typeAnnotation ? assignTypeArguments(call, t.tsTypeParameterInstantiation([field.typeAnnotation.type === "method" ? t.tsFunctionType(undefined, field.typeAnnotation.params.map(p => p.node), t.tsTypeAnnotation(field.typeAnnotation.returnType.node)) : field.typeAnnotation.path.node])) : call)]));
  }
  for (const [, field] of analysis.userDefined.fields) {
    if (field.type === "user_defined_function") {
      ...
      preamble.push(t.variableDeclaration("const", [t.variableDeclarator(t.identifier(field.localName), ts && field.typeAnnotation ? assignTypeArguments(call, t.tsTypeParameterInstantiation([field.typeAnnotation.node])) : call)]));
    } else if (field.type === "user_defined_direct_ref") {
  ...
  if (analysis.effects.cdmPath || analysis.effects.cduPath || analysis.effects.cwuPath) {
    ...
    if (analysis.effects.cwuPath) {
      ...
      preamble.push(t.variableDeclaration("const", [t.variableDeclarator(t.identifier(analysis.effects.cleanupLocalName), assignTypeArguments(t.callExpression(getReactImport("useRef", babel, analysis.superClassRef), [t.nullLiteral()]),
  ...
  for (const softError of softErrors.errors) {
    if (softError.type === "invalid_decl") {
      if (isClassPropertyLike(softError.path) && softError.path.node.value) {
        preamble.push(t.expressionStatement(t.assignmentExpression("=", memberFromDecl(babel, t.identifier("TODO_this"), softError.path.node), softError.path.node.value)));
      } else if (isClassMethodLike(softError.path)) {
        preamble.push(t.expressionStatement(t.assignmentExpression("=", memberFromDecl(babel, t.identifier("TODO_this"), softError.path.node), functionExpressionFrom(babel, softError.path.node))));
      }
    }
  }
  ...
  const params = needsProps(analysis) ? [assignTypeAnnotation(t.identifier("props"),
  ...
  const funcNode = assignTypeParameters(assignReturnType(functionNeeded ? t.functionExpression(analysis.name ? t.cloneNode(analysis.name) : undefined, params, bodyNode) : t.arrowFunctionExpression(params, bodyNode), returnType), analysis.typeParameters?.node);
  return {
    funcNode,
...
function functionDeclarationFrom(babel, node, name) {
  ...
  return assignTypeParameters(assignReturnType(t.functionDeclaration(name ?? functionName(node), node.params, node.body.type === "BlockStatement" ? node.body : t.blockStatement([t.returnStatement(node.body)]), node.generator, node.async), node.returnType), node.typeParameters);
}
function functionExpressionFrom(babel, node, name) {
  const {
    types: t
  } = babel;
  return assignTypeParameters(assignReturnType(t.functionExpression(name ?? functionName(node), node.params, node.body.type === "BlockStatement" ? node.body : t.blockStatement([t.returnStatement(node.body)]), node.generator, node.async), node.returnType), node.typeParameters);
}
...
function arrowFunctionExpressionFrom(babel, node) {
  ...
  return assignTypeParameters(assignReturnType(t.arrowFunctionExpression(node.params, node.body, node.async), node.returnType), node.typeParameters);
}
function constDeclaration(babel, id, init, typeAnnotation) {
  ...
  return t.variableDeclaration("const", [t.variableDeclarator(assignTypeAnnotation(id, typeAnnotation), init)]);
}
